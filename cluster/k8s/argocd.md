...
To use `argocd-vault-plugin`, you need to add it to the pod of `argocd-server` (in the same container or as sidecar, the newer method). We can do it by build a new image base on the original argocd image. First, donwload the binary of `argocd-vault-plugin`:
